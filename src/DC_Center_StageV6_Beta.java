...
@TeleOp(name="DC_Center_StageVBetter")

public class DC_Center_StageV6_Beta extends LinearOpMode{
    ...
    public void runOpMode() {
        ...
        while(opModeIsActive()) {
            ...
            dy = gamepad1.left_stick_y * Math.abs(gamepad1.left_stick_y);
            dx = (-gamepad1.left_stick_x) * Math.abs(-gamepad1.left_stick_x);
            rx = (-gamepad1.right_stick_x) * Math.abs(-gamepad1.right_stick_x);
            robotOrientation = imu.getRobotYawPitchRollAngles();
            heading = robotOrientation.getYaw(AngleUnit.RADIANS);
            if(use_gyro){
                y = dy * Math.cos(heading) - dx * Math.sin(heading);
                x = dy * Math.sin(heading) + dx * Math.cos(heading);
                flpw = Range.clip((y + x + rx)*Cspeed,-1,1);
                frpw = Range.clip((y - x - rx)*Cspeed,-1,1);
                blpw = Range.clip((y - x +  rx)*Cspeed, -1 ,1);
                brpw = Range.clip((y + x - rx)*Cspeed, -1, 1);
            } else {
                flpw = Range.clip((dy + dx + rx)*Cspeed,-1,1);
                frpw = Range.clip((dy - dx - rx)*Cspeed,-1,1);
                blpw = Range.clip((dy - dx +  rx)*Cspeed, -1 ,1);
                brpw = Range.clip((dy + dx - rx)*Cspeed, -1, 1);
            }

            Front_Left_Wheel.setPower(flpw);
            Front_Right_Wheel.setPower(frpw);
            Back_Left_Wheel.setPower(blpw);
            Back_Right_Wheel.setPower(brpw);
...
///////////////////////////////////////////////////////////            //raise and lower the lift to be able to reach higher on the backdrop
            ...
            if (gamepad2.left_trigger >= 0.5) {
                ...
                Position += 300; //Tells lift motor to increase its position by 100
                ...
                telemetry.addData("Lift is at:", Lift_Motor.getCurrentPosition()); //adds telemetry to show where the lift is attempting to get to
                telemetry.update();
                // isbusy
...
//                }
                ...
                Position -= 300; //gets Position variable to get it to change the position lower by 100
                if( Position >= 0) {Position = 0;} //prevents lift motor from going too high
...
///////////////////////////////////////////////////////////
    ...
    private void ReverseDrone(){
        Drone_Motor.setTargetPosition(-250);
        Drone_Motor.setMode(DcMotor.RunMode.RUN_TO_POSITION);

        Drone_Motor.setPower(0.05);
        ...
        while (opModeIsActive() &&
            ...
            telemetry.addData("Goal",  "Running to :%7d", Drone_Motor.getTargetPosition());
            telemetry.addData("Actual distance moved",  "Running at %7d",
                    Drone_Motor.getCurrentPosition());
            telemetry.addData("Speed", Drone_Motor.getPower());
            telemetry.update();
        }


        Drone_Motor.setPower(0);
        Drone_Motor.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
    }
}
